...
class CommissionHoliday(Commission):
    def __init__(self, NewSales):
        Commission.__init__(self, NewSales)
        self.__sales = NewSales
        self.__rate = 0
        self.__commission = 0
...
def main():
    ...
    print("Your Earned Commission is:  $", format(CommCalc.GetCommission(), ',.2f'))
     ...
     #Determine a Rate for the Holiday
    HolidayCommCalc = CommissionHoliday(SalesAmount)
    HolidayCommCalc.DetermineRate()
    HolidayCommCalc.DetermineCommission()


    #print holiday values
    print()
    print("Your total Sales for this holiday was:  $", format(HolidayCommCalc.GetSales(), ',.2f'))
    print("Your Commission Rate is:  ", (HolidayCommCalc.GetRate() * 100), "%")
    print("Your Earned Commission is:  $", format(HolidayCommCalc.GetCommission(), ',.2f'))
